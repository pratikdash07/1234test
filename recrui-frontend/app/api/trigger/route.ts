...
async function triggerLangflowWorkflow(flowId: string) {
  ...
  // CHANGE: Use your backend endpoint for LangFlow integration
  const url = `${langflowBaseUrl}/api/langflow/trigger`
  ...
  try {
    const response = await fetchWithTimeout(
      url,
      {
        ...
        body: JSON.stringify({
          flowId: flowId,
          triggerType: "manual",
          inputPayload: { example: "test" } // Adjust as needed
        }),
      },
      5000,
    )
    ...
    // CHANGE: Map your backend response to the expected structure
    const result = await response.json()
    return {
      success: true,
      run_id: result.runId, // Your backend returns "runId"
