...
export function ExecutionDetailsModal({ open, onOpenChange, executionId, engine }: ExecutionDetailsModalProps) {
  ...
  useEffect(() => {
    if (open && executionId && engine === 'langflow') {
      // Only connect when langflow is selected and modal is open
      const eventSource = new EventSource(`${process.env.NEXT_PUBLIC_API_URL}/api/langflow/runs/${executionId}/stream`)


      eventSource.onmessage = (e) => {
        setRealtimeLogs(prev => [...prev, e.data])
      }
  ...
  const fetchExecutionDetails = async () => {
    ...
    try {
      ...
      const execution = runs.find((run: any) => run.id === executionId);
      if (!execution) {
        throw new Error("Execution not found");
      }
      setExecutionDetails(transformExecutionData(execution, engine));
    } catch (error) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setIsUsingMockData(!process.env.NEXT_PUBLIC_API_URL);
    }
  }, [open, executionId, engine]);
          ...
          <TabsContent value="logs" className="space-y-4">
            <ScrollArea className="h-96">
              ...
              {(executionDetails.logs ?? []).length > 0 ? (
                    ...
                    <Card key={`static-${index}`}>
                      <CardContent className="p-3">
                        <div className="flex items-start gap-2">
                          <Badge
                            variant="outline"
                            className={`text-xs ${log.level === "ERROR" || log.level === "error"
                          ...
                          <div className="flex-1">
                            ...
                            <pre
                              className={`text-sm whitespace-pre-wrap ${log.level === "ERROR" || log.level === "error" ? "text-red-600" : ""
                                }`}
                            >
                ...
                <div className="text-center py-8 text-gray-500">
                  <Info className="w-8 h-8 mx-auto mb-2" />
                  <p>No static logs available for this execution</p>
                </div>
              ) : null}
              ...
              {/* No logs at all */}
              {!(executionDetails.logs?.length) && realtimeLogs.length === 0 && (
                <div className="text-center py-8 text-gray-500">
                  <Info className="w-8 h-8 mx-auto mb-2" />
